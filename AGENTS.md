...
Six focused modules under `src/ibn_monitor/`; no framework, no ORM:
...
| `engine.py` | `PolicyEngine` — evaluates packets against rules using `ipaddress` CIDR matching. Thread-safe via `RLock` (supports live SIGHUP reload). |
...
| `monitor.py` | `MonitorService` — wires everything together; drives Scapy `AsyncSniffer` or offline PCAP. |
| `cli.py` | Four subcommands: `validate`, `check`, `render-nftables`, `run`. |

**Data flow**: Scapy packet → `packet_to_metadata()` (engine) → `PolicyEngine.evaluate()` → matched `Rule` list → `EventDispatcher.dispatch()` → JSONL + webhook queue + metrics.
...
os.environ.setdefault("SCAPY_ROUTE_AUTOLOAD", "0")
```
